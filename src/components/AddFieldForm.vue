<template>
    <q-input type="text" color="primary" v-model="toAdd.label"
    ...
    <q-select filled clearable v-model="toAdd.dataType" use-input hide-selected
    ...
    <q-input v-if="toAdd.dataType && toAdd.dataType !== 'Boolean' && toAdd.dataType !== 'File'" :type="
        ...
        :disabled="toAdd.dataType === null" v-model="toAdd.default" filled clearable
        dense class="col-12 q-my-xs"
        @update:model-value="$emit('ready', toAdd)"
        debounce="500"
    >
        <template v-slot:prepend>
    ...
    <q-select v-else-if="toAdd.dataType === 'Boolean'" filled clearable v-model="toAdd.default" use-input
        ...
        class="col-12 q-px-none q-my-xs" behavior="menu"
        @update:model-value="$emit('ready', toAdd)"
    >
    </q-select>
    <q-input v-if="toAdd.dataType && toAdd.dataType !== 'Boolean'" :type="toAdd.dataType === 'Date' ? 'date' : 'number'"
        ...
        " stack-label v-model="toAdd.minVal" filled clearable dense
        class="col-12 q-my-xs"
        @update:model-value="$emit('ready', toAdd)"
        debounce="500"
    >
    ...
    <q-input v-if="toAdd.dataType && toAdd.dataType !== 'Boolean'" :type="toAdd.dataType === 'Date' ? 'date' : 'number'"
        ...
        " stack-label v-model="toAdd.maxVal" filled clearable dense
        class="col-12 q-my-xs"
        debounce="500"
        @update:model-value="$emit('ready', toAdd)"
    >
    ...
    <q-item class="col-12 row justify-between q-my-xs"
        :class="themeController ? 'form-item-dark' : 'form-item-light'">
        <q-item-section>
            <span>
        ...
        <q-item-section>
            <q-btn-toggle v-model="toAdd.isMandatory" unelevated spread
    ...
    <q-item class="col-12 row justify-between q-my-xs"
        ...
        <q-item-section>
            <q-btn-toggle v-model="toAdd.isUnique" unelevated spread
                toggle-color="secondary" :options="[
                    { label: data[language.getLanguage].yes, value: true },
                    { label: data[language.getLanguage].no, value: false },
                ]"
